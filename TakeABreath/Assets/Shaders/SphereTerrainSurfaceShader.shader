Shader "Custom/SphereTerrainSurfaceShader" {
	Properties {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			else if(dist>_HighMountLevel)
				o.Albedo = c.rgb * _HighMountColor;
			else if(dist >_MountLevel)
				o.Albedo = c.rgb * _MountColor;
			else if(dist >_LowMountLevel)
				o.Albedo = c.rgb * _LowMountColor;
			else if(dist >_LandLevel)
				o.Albedo = c.rgb * _LandColor;
			else if(dist >_BeachLevel)
				o.Albedo = c.rgb * _BeachColor;
			else
				o.Albedo = c.rgb * float4(0.1, 0.1, 0.6, 1);
*/
